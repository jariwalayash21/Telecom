...
Public Class frmsearchold
    Dim conn As New OleDbConnection("Provider=Microsoft.Ace.Oledb.12.0;Data Source=D:\tele\tele\telecom.accdb")
    Dim ds As New DataSet
    Private Sub frmsearchold_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub populate(Optional ByVal mode As Integer = 0)
        ...
        Try
            If mode = 1 Then
                cmd.CommandText = "Select * from cust_old order by cid"
            ...
            Else
                If RadioButton2.Checked Then
                    cmd.CommandText = "Select * from cust_old where cid=" & CInt(TextBox29.Text)
                    da.SelectCommand = cmd
                    da.Fill(ds)
                    fillDataGrid(ds)
                ElseIf RadioButton3.Checked Then
                    Dim da2 As New OleDbDataAdapter
                    Dim cmd2 As New OleDbCommand("Select cid from cust_conn_old where cnum=" & Val(TextBox29.Text), conn)
                    Dim ds2 As New DataSet
                    da2.SelectCommand = cmd2
                    da2.Fill(ds2)
                    cmd.CommandText = "Select * from cust_old where cid=" & ds2.Tables(0).Rows(0).Item(0)
                    da.SelectCommand = cmd
                    da.Fill(ds)
                    fillDataGrid(ds)
                ElseIf RadioButton1.Checked Then
                    Dim cname As String = TextBox29.Text
                    cmd.CommandText = "Select * from cust_old where cfname='" & cname & "' or cmname='" & cname & "' or clname='" & cname & "'"
                    da.SelectCommand = cmd
                    da.Fill(ds)
                    fillDataGrid(ds)
                Else
                    cmd.CommandText = "Select * from cust_old where cid in (Select cid from cust_conn_old where billstatus='Unpaid')"
                    da.SelectCommand = cmd
                    da.Fill(ds)
                    fillDataGrid(ds)
                End If
    ...
    Private Sub gotClick(ByVal r_index As Integer)
        ...
        Dim da10 As New OleDbDataAdapter("Select ctype,cnum,plan,billstatus,reg_date from cust_conn_old where cid=" & CInt(Label1.Text), conn)
